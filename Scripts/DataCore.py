...
class ClassificationArrayDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with fits.open(file_path) as hdul:
            ...
            if img_data.ndim > 2:
                img_data = img_data[0]
            img_data = img_data.astype(np.float32).newbyteorder("=")

        img_data = self.transform(img_data) 
        # Convert to tensor and replicate channel => shape [3, H, W]
        img_tensor = torch.tensor(img_data).unsqueeze(0).repeat(3, 1, 1)

        # 2D label: [is_pre_or_not, is_post_or_not]
        is_pre = (row['is_pre_merger'] == 1)
        is_ongoing = (row['is_ongoing_merger'] == 1)
        is_post = (row['is_post_merger'] == 1)
        labelList = [0, 0]
        ...
        if is_ongoing or is_post:
            labelList[1] = 1
        label_tensor = torch.tensor(labelList, dtype=torch.float)  # float for BCEWithLogitsLoss
